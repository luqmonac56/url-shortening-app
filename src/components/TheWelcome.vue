<template>
  ...
  <section class="  relative px-4  md:px-12 pb-[8rem] bg-[#bfbfbf36] mt-[10rem]">
    <form @submit.prevent="getShortLink" class="input-div px-4 py-8 lg:p-[3.5rem] flex flex-col lg:flex-row gap-4 lg:gap-8 relative -top-[5rem]">
      <input v-model="longLink"  type="text" placeholder="Shorten a link here..." >
    ...
    <div v-if="links.full_short_link2" class=" rounded-2xl mb-8 display-results px-4 py-4 lg:p-[2.5rem] flex flex-col lg:flex-row justify-between items-center gap-4 lg:gap-8 shadow-2xl bg-white">
